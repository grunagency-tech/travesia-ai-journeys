...
import travelSearchApi from "@/lib/api/travelSearch";
      ...
      if (data.status === "complete") {
        try {
          ...
          if (itinerary) {
            ...
            // Enrich hotel options with real data from Hotellook API
            try {
              const dest = mergedTripData.destino || tripDestination || '';
              const checkin = mergedTripData.fechaSalida || tripDate || '';
              const checkout = mergedTripData.fechaRegreso || tripEndDate || '';
              const pax = mergedTripData.pasajeros || tripTravelers || 2;
              
              if (dest && checkin && checkout) {
                console.log('Enriching hotels with real data for:', dest);
                ...
                if (hotelResult.hotels && hotelResult.hotels.length > 0) {
                  console.log(`Got ${hotelResult.hotels.length} real hotels, merging into itinerary`);
              ...
              console.warn('Hotel enrichment failed, using AI-generated data:', hotelErr);
            }
  ...
  return (
    <div className="h-screen flex">
      ...
      {user && showDesktopSidebar && (
        <div className="hidden lg:flex w-72 border-r border-border bg-gray-50 flex-col flex-shrink-0">
          <ConversationList 
      ...
      {/* Chat Section */}
      <div className={`${showContentOnMobile ? 'hidden md:flex' : 'flex'} w-full md:basis-[40%] md:max-w-[40%] flex-col bg-gradient-to-b from-gray-50 to-white md:flex-shrink md:min-w-[300px]`}>
      ...
      <div className={`${showContentOnMobile ? 'fixed inset-0 z-50 flex flex-col' : 'hidden md:flex md:flex-col'} w-full md:relative md:flex-1 bg-background md:bg-primary items-stretch md:items-center justify-center md:p-6 overflow-hidden`}>
        ...
        {itineraryData ? (
          ...
          <LoadingItinerary 
            destination={tripDestination || undefined} 
            t={t} 
            externalProgress={isGeneratingItinerary ? generationProgress : undefined}
          />
        )}
      </div>
    </div>
